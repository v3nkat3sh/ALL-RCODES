...
Combined_Data$FY<-"FY18"

##Remove all the entries for June (Month 6)
...
FY18_Media_Week_Zero<-subset(Prefinal_Data, Media.Week == 0)
FY18_Data_wo_MWZero<-subset(Prefinal_Data, Media.Week != 0)

##Reclassify all media week zeros into media week 52 along with the dependent variables
FY18_Media_Week_Zero$Quarter<-'Q4'
FY18_Media_Week_Zero$MonthNum<-12
FY18_Media_Week_Zero$Year<-2017
FY18_Media_Week_Zero$Media.Week<-52
...
setwd('D:/WORK/R RELATED/XBOX & OCC in R/OCC/WEEKLY FY18/OUTPUTS')
write.csv(Final_Data, file = paste("OCC_Weekly_Report_Output",Sys.Date(),"csv",sep = '.'))
...
##Adding the PC/Tab/Phone Column to the Final Classified Datafile##
###Using Lookup Tab
##LookUpTab Classification by Hardware
#DataForLookUpTab<-Prefinal_Data
