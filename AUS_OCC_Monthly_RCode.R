###### AUS Monthly, OCC REPORTING RCODE ######
JulytoSep<-Australia_Monthly_Report_Sep_2016[,c(1:28)]
Oct<-AUS_Monthly_Reports_Oct_2016[,c(1:28)]
Nov<-Microsoft_International_Australia_Report_November_2016[,c(1:28)]
...
Category_Financial<-subset(Data1,Category == "FINANCIAL")
Category_MediaPublishing<-subset(Data1,Category == "MEDIA AND PUBLISHING")
...
Data<-rbind(Category_ComputerComponentsPeripherals,Category_Computer_Hardware_Excl_Network,Category_Computer_Software,Category_Financial,Category_MediaPublishing,Category_Telecommunications)
...
setwd('D:/WORK/R RELATED/XBOX & OCC in R/OCC/INTERNATIONAL/AUS/OUTPUT')
write.csv(Final_Data, file = paste("OCC_Monthly_AUS_Report_Output",Sys.Date(),"csv",sep = '.'))
